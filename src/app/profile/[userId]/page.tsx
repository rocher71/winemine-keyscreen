...
import { getUserById, currentUserHeavy } from '@/lib/mock/users';
...
export default function OtherProfilePage() {
  ...
  const other = getUserById(params.userId);

  const sortedWines = useMemo(() => {
    if (!other) return [];
    const notes = getTastingNotesByUser(other.id);
  ...
  const sharedStats = useMemo(() => {
    ...
    const myNotes = getTastingNotesByUser(currentUserHeavy.id);
    const otherNotes = getTastingNotesByUser(other.id);
  ...
  const matchPct = getCompatibility(currentUserHeavy, other);
